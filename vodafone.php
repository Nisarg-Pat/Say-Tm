...
<center>
<div>
<img src="vodafone.png" width="100" height="75"><br>
<div style="font-size: 30px">Vodafone</div>
</div>
</center>
<hr>
...
<caption style="text-align: center;color: black;font-size: 20px">Select Mobile plan</caption>
	...
	<?php
  	include("conn.php");
  	$s="select * from plans where oid=1 and ptype='mobile'";
  	$res=$conn->query($s);
...
<caption style="text-align: center;color: black;font-size: 20px">Select Data plan</caption>
	...
	<?php
  	include("conn.php");
  	$s="select * from plans where oid=1 and ptype='data'";
  	$res=$conn->query($s);
  	while ($fe=$res->fetch_object())
  	{
  	?>
