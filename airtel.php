...
<center>
<div>
<img src="airtel.png" width="100" height="75"><br>
<div style="font-size: 30px">Airtel</div>
</div>
</center>
<hr>
...
<caption style="text-align: center;color: black;font-size: 20px">Select Mobile plan</caption>
	...
	<?php
  	include("conn.php");
  	$s="select * from plans where oid=3 and ptype='mobile'";
  	$res=$conn->query($s);
...
<caption style="text-align: center;color: black;font-size: 20px">Select Data plan</caption>
	...
	<?php
  	include("conn.php");
  	$s="select * from plans where oid=3 and ptype='data'";
  	$res=$conn->query($s);
  	while ($fe=$res->fetch_object())
  	{
  	?>
